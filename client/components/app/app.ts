...
export class App {
    ...
    constructor() {
        ...
        this.typeahead = new Awesomplete(input);
        this.typeahead.list = [];
        ...
        Rx.Observable.fromEvent(input, 'keyup')
            ...
            .debounce(250)
            .flatMapLatest((term) => {
                return window.fetch(`api/tags?matchName=${encodeURIComponent(term)}&max=10`).then(res => res.json());
            })
            .map((matchingTags) => {
                return matchingTags.map(tag => tag.name);
            })
            .subscribe((matchingTags) => {
                this.typeahead.list = matchingTags;
                console.log(matchingTags);
            });

        input.addEventListener('awesomplete-selectcomplete', (event) => {
            console.log('wrt', event);
            window.fetch(`api/tagByName/${encodeURIComponent(event.target.value)}`)
                .then(res => res.json())
                .then((tagData) => {
